...
.text
main:
	addi t0, x0, 0   # add immediate        t0 = x0 + 0, that is, set t0 to 0
	la t1, source    # load address         t1 = &source
	la t2, dest	     # load address         t2 = &dest
loop:
	slli t3, t0, 2   # shift left immediate t3 = t0 << 2, that is, t3 = t0 * 4
	add t4, t1, t3   # add                  t4 = t1 + t3
	lw t5, 0(t4)     # load word            t5 = *t4
    slli t5, t5, 1  # shift left immediate  t5 = t5 << 1, that is, t5 = t5 * 2
	beq t5, x0, exit # branch equal         t5 == x0, if *t4 == 0, jump to label exit
	add t6, t2, t3   # add                  t6 = t2 + t3
	sw t5, 0(t6)     # store word           *t6 = t5
	addi t0, t0, 2   # add immediate        t0 = t0 + 2
	jal x0, loop     # jump and link        goto label loop
exit:
	jal ra, print_lists
	addi a0, x0, 10
	add a1, x0, x0
