...
/* (To the TAs:) you should remove the original comment here from the code
 * template, since this function *should* be called myself. */
...
static int add_if_label(uint32_t input_line, char *str, uint32_t byte_offset,
  ...
  return add_to_table(symtbl, str, byte_offset) == -1 ? -1 : 1;
}
...
int pass_one(FILE *input, Block *blk, SymbolTable *table) {
  ...
  while (fgets(buf, BUF_SIZE, input)) {
    ...
    if (!tok) {
      ...
      /* Label */
      int lbl_result = add_if_label(blk->line_number, tok, offset, table);
      if (lbl_result == 0) {
        /* Not a label, treat it as an instruction */
        tokens[tok_count++] = tok;
      } else if (lbl_result == -1) {
        /* Invalid label */
        error = -1;
      }
    }
    ...
    while (tok) {
      if (tok_count >= MAX_ARGS + 1) {
        raise_extra_argument_error(blk->line_number, tok);
        error = -1;
        valid_insn = false;
        break;
      }
    ...
    char **args = tokens + 1;
    int args_count = tok_count - 1;

    /* Instructions */
    if (valid_insn) {
      int write_result = write_pass_one(blk, tokens[0], args, args_count);
      if (write_result == 0) {
        raise_instruction_error(blk->line_number, tokens[0], args, args_count);
        error = -1;
      } else {
        offset += INSN_SIZE * write_result;
      }
    }
...
int pass_two(Block *blk, SymbolTable *table, FILE *output) {
  ...
  for (uint32_t i = 0; i < blk->len; ++i) {
    ...
    int result = translate_inst(output, inst->name, inst->args, inst->arg_num,
                                addr, table);
    if (result != 0) {
      raise_instruction_error(inst->line_number, inst->name, inst->args,
                              inst->arg_num);
      error = -1;
    }
  }
