...
int translate_num(long int *output, const char *str, ImmType type) {
  ...
  int result = read_imm(str, output);
  if (result == -1 || !is_valid_imm(*output, type)) {
    return -1;
  }
  return 0;
}
...
/* (To the TAs:) Please double check if there are still any parts you intend to
 * delete remaining before you release the project template. */
...
int translate_reg(const char *str) {
  ...
  size_t reg_map_size = sizeof(reg_map) / sizeof(RegEntry);
...
int is_valid_imm(long imm, ImmType type) {
  ...
  case IMM_NONE:
    return 1;
  case IMM_12_SIGNED:
    return imm >= ((1 << 11) * -1) && imm < (1 << 11);
  case IMM_13_SIGNED:
    return imm >= ((1 << 12) * -1) && imm < (1 << 12);
  case IMM_21_SIGNED:
    return imm >= ((1 << 20) * -1) && imm < (1 << 20);
  case IMM_5_UNSIGNED:
    return imm >= 0 && imm < (1 << 5);
  case IMM_20_UNSIGNED:
    return imm >= 0 && imm < (1 << 20);
  default:
    return 0;
  }
}
