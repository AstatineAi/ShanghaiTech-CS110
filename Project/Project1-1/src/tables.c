...
SymbolTable *create_table(int mode) {
  ...
  table->entries = malloc(INCREMENT_OF_CAP * sizeof(Symbol));
  if (!table->entries) {
    free(table);
    allocation_failed();
  }
...
static char *strdup_e(const char *src) {
  if (!src) {
    return NULL;
  }
...
int add_to_table(SymbolTable *table, const char *name, uint32_t addr) {
  ...
  table->entries[table->len].name = strdup_e(name);
  if (!table->entries[table->len].name) {
    allocation_failed();
  }
...
void resize_table(SymbolTable *table) {
  ...
  Symbol *new_entries =
      realloc(table->entries, (table->cap + INCREMENT_OF_CAP) * sizeof(Symbol));
  if (!new_entries) {
    allocation_failed();
  }
