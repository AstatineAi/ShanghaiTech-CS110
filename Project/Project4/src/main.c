...
int main(void) {
  ...
  int ret = scenario_select(1);
  if (ret == 0) {
    LCD_Clear(BLACK);

    char str[20];
    sprintf(str, "Scenario %d", scene_number);
    LCD_ShowString(5, 5, (u8*)str, WHITE);

    delay_1ms(300);
    LCD_Clear(BLACK);

    GameState game;
    init_game(&game, scene_number);

    while (1) {
      update_game(&game);
      draw_game(&game);
      delay_1ms(40);  // Target ~25 FPS
    }
  } else {
    char str[20];
    sprintf(str, "Error: %d", ret);
    LCD_ShowString(5, 5, (u8*)str, RED);
    delay_1ms(2000);
  }
  return 0;
}
