...
\begin{answer}[Question 3]
    ...
    \begin{tikzpicture}[->, >={Stealth[scale=2.5]}]
        ...
        \node[state, minimum size=15mm] (s3) at (3.8,-3.2) {01};
        \node[state, minimum size=15mm] (s2) at (-3.8,-3.2) {00};

        \draw (s1) edge [below] node {1 / 0} (s3);
        \draw (s1) edge [below] node {0 / 0} (s2);
        \draw (s2) edge [loop left] node {0 / 1} (s2);
        \draw (s3) edge [loop right] node {1 / 1} (s3);
        \draw (s3) edge [bend right] node {1 / 0} (s2);
        \draw (s2) edge [bend right] node {0 / 0} (s3);
    \end{tikzpicture}
    \end{center}
